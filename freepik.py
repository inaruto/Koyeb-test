...
class Freepik:
    ...
    def _download_url_from_id(self, id_: str, captcha_token: str = None, retries: int = 1):
        url = f'https://www.freepik.com/xhr/download-url/{id_}'
        params = {}
        if captcha_token:
            params = {'token': captcha_token}
        resp_json = self.session.get(
            url, headers=self.xhr_headers, params=params).json()
        if resp_json.get('success'):
            ...
            if captcha_token:
                raise RuntimeError(
                    f'Manual validation did not work {resp_json}')
            manual_validation_resp = self.session.get(
                resp_json['url'], headers=self.xhr_headers)
            ...
            if self.on_premium_end:
                ...
                raise RuntimeError(
                    f'The premium subscription ended {resp_json}')
        else:
...
if __name__ == '__main__':
    freepik = Freepik(os.environ['FREEPIK_USERNAME'],
                      os.environ['FREEPIK_PASSWORD'], os.environ['TWO_CAPTCHA_API_KEY'])
